(ns marketentry.marketentryllm
  ...
  the IDN public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-id-entity?=" (:has-id-entity? e)
                        " nib-verified?=" (:nib-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-id-entity? e))
                              (:has-id-entity? e))
                          (or (not (:requires-nib? e))
                              (:nib-verified? e)))
                   0.9 0.3)}))
