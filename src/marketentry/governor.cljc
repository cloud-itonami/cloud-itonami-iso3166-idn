(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether an Indonesia-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Indonesia-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-id-entity?
                                       true` (near-universal for Indonesia
                                       ...
                                       Grounded in Indonesian legal entity
                                       (NIB) typically required for SPSE
                                       participation (LKPP / contracting
                                       authorities).
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(NIB/統一資格/SPSE登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- id-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-id-entity? true`, INDEPENDENTLY verify
  `:has-id-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-id-entity?` ground truth (most Indonesia public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-id-entity? e))
                 (not (true? (:has-id-entity? e))))
        [{:rule :id-entity-missing
          :detail (str subject " はインドネシア居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- nib-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-nib? true`, INDEPENDENTLY check
  `:nib-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-nib? e))
                 (not (true? (:nib-verified? e))))
        [{:rule :nib-unverified
          :detail (str subject " はNIB確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (id-entity-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (nib-unverified-violations request st)
